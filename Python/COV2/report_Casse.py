...
class Case():
    ...
    def testWorkload(self, yesterDate, intDate, reportDate):
        ...
        # 百分比
        percent = '{:.1f}%'.format(abs(differenceData / datawork[0] * 100))
        # print(percent)
        # 医疗、疾控和第三方实验室占比
        differenceOrgansql = [
            {
        ...
        if differenceData < 0:
            ...
            print(reportDate + '全国新冠病毒核酸检测约' + str(aroundData) + '万人份,' + '较前一日增加了' +
                  str(aroundDifference) + '万人份,' + '升高了' + percent)
            print('其中医疗机构检测约' + str(aroundMedical) + '万人份（占' + medicalPercent + '）,疾控机构约' + str(
                aroundCDC) + '万人份（占' + CDCPercent + '),'
                                                    '' + '第三方实验室约' + str(
                aroundThirdlab) + '万人份（占' + thirdLabpercent + ')')
    ...
    def testSpecialpovince(self, testDate, inputName, intDate, reportDate, yesterdate, intStartdate, realStartdate):
        ...
        print('共检测样本' + str(aroundTotalservings) + '万人份,其中医疗机构检测' + str(
            ...
            aroundThirdlabServings) + '万人份占(' + thirdLabpercent + ')。')
        #   省份检测量
        workloadCol = [{
            '$match': {
                'testDate': {'$gte': yesterdate, '$lte': intDate},
        ...
        aroundStartworkload = round(startWorkload / 10000, 1)
        startPercent = '{:.1f}%'.format(abs(testLoad[1] - startWorkload) / startWorkload * 100)
    ...
    def testSpecialCity(self, testDate, inputName, intDate, reportDate, yesterdate, intStartdate, realStartdate):
        testLoad = []
        compareDateresult = []
        # 全市所有医疗卫生机构
        citySql = "SELECT  t1.city_code,t3.city_name,COUNT(*)  from report_workload_record t1 join report_lab_info_ext t2 " \
                  "on t1.lab_id = t2.lab_id join common_city t3 on t3.city_code = t1.city_code  " \
                  "where  t1.flow_status = 2 and t1.test_day = '%s' and t3.city_name like '%%%s%%'" \
                  "and t2.inst_function in('inst_function_0002','inst_function_0003') GROUP BY t1.city_code" % (
                      testDate, inputName)

        cityResult = self.Common.connectMySQL(citySql)
        # print(provinceResult)
        cityCode = cityResult[0][0]
        cityName = cityResult[0][1]
        cityTotal = cityResult[0][2]

        # 全市医疗机构
        medicalSql = "SELECT COUNT(*)  from report_workload_record t1 join report_lab_info_ext t2 on t1.lab_id = t2.lab_id " \
                     "join common_city t3 on t3.city_code = t1.city_code  where  t1.flow_status = 2 and " \
                     "t1.test_day ='%s' and t3.city_name like '%%%s%%' and t2.inst_function " \
                     "in('inst_function_0002','inst_function_0003')  and t2.inst_category = 'inst_category_0001'" % (
                         testDate, inputName)
        ...
        totalServings = self.Common.connectMongDB(cityCol)
        for i in totalServings:
            totalServings = i['totalwork']
        aroundTotalservings = round(totalServings / 10000, 1)
        #  医疗机构样本人份
        ...
        aroundThirdlabServings = round(thirdLabservings / 10000, 1)
        thirdLabpercent = '{:.1f}%'.format(thirdLabservings / totalServings * 100)
        print(reportDate + '0时至24时,我委收到' + cityName + str(
            cityTotal) + '家卫生医疗机构上报新冠病毒核酸检测数据，其中医疗机构' + str(
            medicalResult) + '家,疾控机构' + str(CDCResult) + '家,第三方实验室' + str(thirdLabresult) + '家.')
        print('共检测样本' + str(aroundTotalservings) + '万人份,其中医疗机构检测' + str(
            aroundMedicalservings) + '万人份（占' + medicalPercent + ')，疾控机构检测' + str(
            aroundCDCservings) + '万人份，（占' + CDCpercent + '），第三方实验室检测' + str(
        ...
        workloadPercent = '{:.1f}%'.format(abs(compareData) / testLoad[0] * 100)
        if compareData < 0:
            print(cityName + '检测量达' + str(aroundTestload) + '万人份,比前一日' + '(' + str(
                aroundYesterdayresult) + '万人份)降低了' + workloadPercent)
        else:
            print(cityName + '检测量达' + str(aroundTestload) + '万人份,比前一日' + '(' + str(
                aroundYesterdayresult) + '万人份)升高了' + workloadPercent)
        ...
        startWorkload = self.Common.connectMongDB(workloadCol)
        realStartWorkload = 0
        if not startWorkload:
            startWorkload = 0
        else:
            for i in startWorkload:
                realStartWorkload = i['totalwork']
        aroundStartworkload = round(realStartWorkload / 10000, 1)
        startPercent = '{:.1f}%'.format(abs(testLoad[1] - realStartWorkload) / realStartWorkload * 100)
        if testLoad[1] - realStartWorkload < 0:
            print('比' + realStartdate + '(' + str(aroundStartworkload) + '万人份' + ')降低了' + startPercent)
        else:
            print('比' + realStartdate + '(' + str(aroundStartworkload) + '万人份' + ')升高了' + startPercent)

    def testSpecialArea(self, testDate, inputName, intDate, reportDate, yesterdate, intStartdate, realStartdate):
        testLoad = []
        compareDateresult = []
        realThirdServings = 0
        # 全区县所有医疗卫生机构
        areaSql = "SELECT t4.city_name,t1.area_code,t3.area_name,COUNT(*)  from report_workload_record t1 join report_lab_info_ext t2 " \
                  "on t1.lab_id = t2.lab_id join common_area t3 on t3.area_code = t1.area_code join common_city t4 " \
                  "on t3.city_code = t4.city_code where  t1.flow_status = 2 and t1.test_day = '%s' and t3.area_name like '%%%s%%'" \
                  "and t2.inst_function in('inst_function_0002','inst_function_0003') GROUP BY t1.area_code" % (
                      testDate, inputName)

        areaResult = self.Common.connectMySQL(areaSql)
        # print(provinceResult)
        cityName = areaResult[0][0]
        areaCode = areaResult[0][1]
        areaName = areaResult[0][2]
        areaTotal = areaResult[0][3]

        # 全区县医疗机构
        medicalSql = "SELECT COUNT(*)  from report_workload_record t1 join report_lab_info_ext t2 on t1.lab_id = t2.lab_id " \
                     "join common_area t3 on t3.area_code = t1.area_code  where  t1.flow_status = 2 and " \
                     "t1.test_day ='%s' and t3.area_name like '%%%s%%' and t2.inst_function " \
                     "in('inst_function_0002','inst_function_0003')  and t2.inst_category = 'inst_category_0001'" % (
                         testDate, inputName)
        ...
        totalServings = self.Common.connectMongDB(areaCol)
        for i in totalServings:
            totalServings = i['totalwork']
        aroundTotalservings = round(totalServings / 10000, 1)
        #  医疗机构样本人份
        ...
        medicalServings = self.Common.connectMongDB(medicalCol)
        realmedicalServings = 0
        if not medicalServings:
            realmedicalServings = 0
        else:
            for i in medicalServings:
                realmedicalServings = i['totalwork']
        aroundMedicalservings = round(realmedicalServings / 10000, 1)
        medicalPercent = '{:.1f}%'.format(realmedicalServings / totalServings * 100)
        #   疾控样本人份
        CDCcol = [{
            '$match': {
                'testDate': intDate,
        ...
        if not CDCservings:
            ...
            for i in CDCservings:
                realCDCservings = i['totalwork']
        aroundCDCservings = round(realCDCservings / 10000, 1)
        # print(aroundCDCservings)
        CDCpercent = '{:.1f}%'.format(realCDCservings / totalServings * 100)
        #   第三方实验室样本人份
        thirdLabcol = [{
            '$match': {
                'testDate': intDate,
        ...
        thirdLabservings = self.Common.connectMongDB(thirdLabcol)
        # for i in thirdLabservings:
        #     realThirdServings = i['totalwork']
        if thirdLabservings == "":  ##判断查询结果是否为空
            realThirdServings = 0
        else:
            for i in thirdLabservings:
                realThirdServings = i['totalwork']
        aroundThirdlabServings = round(realThirdServings / 10000, 1)
        thirdLabpercent = '{:.1f}%'.format(realThirdServings / totalServings * 100)
        print(thirdLabpercent)
        print(reportDate + '0时至24时,我委收到'+cityName + areaName + str(
            areaTotal) + '家卫生医疗机构上报新冠病毒核酸检测数据,其中医疗机构' + str(
            medicalResult) + '家,疾控机构' + str(CDCResult) + '家,第三方实验室' + str(thirdLabresult) + '家.')
        print('共检测样本' + str(aroundTotalservings) + '万人份,其中医疗机构检测' + str(
            aroundMedicalservings) + '万人份（占' + medicalPercent + ')，疾控机构检测' + str(
            aroundCDCservings) + '万人份(占' + CDCpercent + '),第三方实验室检测' + str(
        ...
        workloadPercent = '{:.1f}%'.format(abs(compareData) / testLoad[0] * 100)
        if compareData < 0:
            print(areaName + '检测量达' + str(aroundTestload) + '万人份,比前一日' + '(' + str(
                aroundYesterdayresult) + '万人份)降低了' + workloadPercent)
        else:
            print(areaName + '检测量达' + str(aroundTestload) + '万人份,比前一日' + '(' + str(
                aroundYesterdayresult) + '万人份)升高了' + workloadPercent)
