import { checkDisplayFields, checkRadio, clickSearch, clickTool, inputSearch, openProvince, openTagSelect, validCclChecked, validCclSelect, validCellText, validMonthRange, validPrint, validRowsLength } from '../../shared/effect-query'
import { activeSelect, clickListener, interceptAll, validatePdfFile, waitIntercept } from '../../shared/util'
import { changeCcl, shCclName, tagName } from '../../shared/stats-query'
...
describe('实验室上报情况', () => {
  ...
  const queryApear = () => {
    return interceptAll('service/mgr/evaReport/labReport?*', 'queryApear')
  }
  let result
  const waitQuery = (cb) => {
