import { changeCcl, waitQueryIntercept, checkRegion, checkSpec, activeSomeSpec, regionText, specText, shCclName, specName, itemName, queryYear, startMonth, tagName, buttonPushScreen, buttonPreview, interceptCcl, interceptSpec, reportMonth, pushToScreen } from '../../shared/stats-query'
import { getUrlQuery, interceptAll, interceptPost, waitIntercept } from '../../shared/util'
...
context('告警及反馈统计页面', () => {
  ...
  before(() => {
    ...
    waitIntercept(interceptCcl, data => {
  ...
  context('查询前置条件', () => {
    ...
    context(`认可专业选择${specName}，日期选择${weekText}`, () => {
      ...
      it(`006-互认项目选择${itemName}，校验返回的项目`, () => {
        ...
        cy.elSelectActive(itemName)
  
        validItemResult(itemName)
      })

      it('007-切换到管理机构或实验室标签', () => {
        cy.get('@groupSpec').find('.el-select__tags').click({
          force: true
        })
        cy.elSelectActive(specName)
      ...
      it(`008-${alarmType}`, () => {
        ...
        cy.elSelectActive('未上报')

        waitQuery((data) => {
          expect(data.labs).to.exist
          // 重置
          openSelect(alarmType)
          cy.elSelectActive('全部')
        })
      ...
      it(`009-${feedbackStatus}`, () => {
        ...
        cy.elSelectActive('已知晓')

        waitQuery((data) => {
          expect(data.labs).to.exist
          // 重置
          openSelect(feedbackStatus)
          cy.elSelectActive('未读')
        })
