...
context('结果互认设置-开展项目设置', () => {
  ...
  before(() => {
    ...
    cy.visit('/cqb-base-mgr-fe/app.html#/mutual-std/mutual-item')
    cy.wait('@getData')
  })
  context('项目分类操作', () => {
    const specItemName = '自动化测试' + mathRomdomNumber(0, 1000)
    const newName = '自定义名称' + mathRomdomNumber(0, 100)
    before(() => {
      cy.wait(1000)
    })
    it('添加相同的项目分类', () => {
    ...
    it('添加不存在的项目分类', () => {
      addItemType('自定义分类', specItemName)
      waitRequest({
        ...
        onSuccess: () => {
          cy.get('.item-configNew__aside-body').should('contain', specItemName)
        }
      })
    })
    it('编辑项目分类', () => {
      cy.get('.item-configNew__aside-body').contains(specItemName).parent('.el-menu-item').find('.el-icon-edit').click({
        force: true
      })
      cy.wait(2000)
      //编辑已存在的名称不允许保存
      getDialog('编辑项目分类').within(() => {
      ...
      //编辑新的名称可以保存
      getDialog('编辑项目分类').within(() => {
        elform('tagName').clear().type(newName)
      })
      withinDialog(clickOkInDialog, '编辑项目分类')
      withinDialog(clickOkInDialog, '编辑项目分类')
      cy.get('.item-configNew__aside-body').should('contain', newName)
    })
    it('删除项目分类', () => {
      cy.get('.item-configNew__aside-body').contains(newName).parent('.el-menu-item').find('.el-icon-delete').click({
        force: true
      })
      cy.wait(1000)
      waitIntercept(interceptDeleteItemSpec, () => {
        ...
        cy.get('.item-configNew__aside-body').should('not.contain', newName)
      })
    })
  })
  context('项目操作', () => {
  ...
  context('其他操作', () => {
    ...
    it('项目搜索', () => {
      cy.get('.item-configNew__aside-body').contains('全血细胞计数').click({
        force:true
      })
      cy.wait(3000)
    ...
    it('配置项目TEa', () => {
      waitIntercept(queryItemTeaSource, () => {
        ...
        if (itemTea.length) {
          ...
          activeSelect(itemTea[0].sourceName, false)
          waitIntercept(interceptAddItem, () => {
            withinDialog(clickOkInDialog, '编辑项目')
          }, () => {
