context('月度工作日申请审核', () => {
  ...
  it('0005-工作日申请-使用项目进行搜索', () => {
    ...
    cy.intercept('**/cqb-base-mgr/service/mgr/item/workDays/page?*').as('getIteamStatus')
    cy.get('.el-button.el-button--primary.el-button--medium').eq(searchButton).click({
      force: true
    })
    cy.wait('@getIteamStatus').then((xhr) => {
      let expectStatus = 200
      let totalData = xhr.response.body.data.total
      let getStatus = xhr.response.statusCode
      expect(getStatus).to.eq(expectStatus)
    ...
    cy.intercept('**/cqb-base-mgr/service/mgr/item/workDays/page?*').as('getIteamStatusPass')
    cy.get('.el-button.el-button--primary.el-button--medium').eq(searchButton).click({
      force: true
    })
    cy.wait('@getIteamStatusPass').then((xhr) => {
      let expectStatus = 200
      let totalData = xhr.response.body.data.total
      let getStatus = xhr.response.statusCode
      expect(getStatus).to.eq(expectStatus)
    ...
    cy.intercept('**/cqb-base-mgr/service/mgr/item/workDays/page?*').as('getIteamStatusUnpass')
    cy.get('.el-button.el-button--primary.el-button--medium').eq(searchButton).click({
      force: true
    })
    cy.wait('@getIteamStatusUnpass').then((xhr) => {
      let expectStatus = 200
      let totalData = xhr.response.body.data.total
      let getStatus = xhr.response.statusCode
      expect(getStatus).to.eq(expectStatus)
