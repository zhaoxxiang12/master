...
describe('账户管理-实验室账户', function () {
    ...
    it('001-添加实验室-实验室名称为空不能保存', () => {
        ...
        cy.wait(1000)
        // 关闭窗口
        cy.get('[aria-label="添加实验室"] .el-dialog__footer button').eq(closeButton).click({force: true})
    })
    ...
    it('010-编辑实验室-修改实验室编码(相同)', () => {
        ...
        //断言
        cy.get('body').should('contain', '编码已存在，请重新输入')
        cy.get('[aria-label="编辑实验室"] .el-dialog__footer button').eq(closeButton).click({force: true})
    })
