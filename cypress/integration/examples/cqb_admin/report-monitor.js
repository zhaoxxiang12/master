context('监控内容配置', () => {
    ...
    it('008-监控内容配置-所在地进行搜索(贵州)', () => {
        ...
        //选择北京市
        cy.wait(500)
        cy.get('.el-menu').eq(areaBox).find('li').eq(Guizhou).find('.el-checkbox__inner').click({
            force: true
        })
        cy.server()
        cy.route('**/service/mgr/new/reportmonitors/*').as('getData')
    ...
    //     })
    //     cy.server()
    //     cy.route('**/service/mgr/iqccenter/loginByDate??*').as('getData')
    //     cy.visit(urlHost + '#/setting/report-monitor')
