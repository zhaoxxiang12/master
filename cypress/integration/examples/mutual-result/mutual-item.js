...
context('结果互认设置-开展项目设置', () => {
    ...
    it('012-开展项目设置-添加自定义项目(已存在)', () => {
        ...
        cy.get('.el-scrollbar__view.el-select-dropdown__list').eq(DropDownList).find('.el-select-dropdown__item').eq(custom).click({
            force: true
        })
    ...
    it('019-开展项目设置-质控主管单位切换至青浦医联体', () => {
        ...
        cy.get('.el-scrollbar__view.el-select-dropdown__list').eq(DropList).find('li').eq(Foshan).click({
            force: true
        })
    })
