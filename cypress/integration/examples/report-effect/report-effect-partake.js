context('参与实验室情况', () => {
  ...
  it('001-参与实验室情况-未参与标签实验室数', () => {
    ...
    //断言
    cy.get('body').should('contain', '佛山市高明区妇幼保健院')
  })
  ...
  it('005-参与实验室情况-数据校验', () => {
    ...
    if (stringApplyRate.length == 1) {
      applyRate = applyRate.toString() + ".000"
    }
    if (stringApplyRate.length > 1) {
    ...
    if (stringNotJoinedRate.length == 1) {
      notjoinedRate = notjoinedRate.toString() + ".000"
    }
    if (stringNotJoinedRate.length > 1) {
  ...
  it('008-参与实验室情况-使用地区进行搜索(贵州省贵阳市)', () => {
    let areaIndex = 0
    let boxIndex = 4
    let cityIndex = 4
    let guizhouIndex = 2
    let guiyangCityindex = 0
    let resultIndex = 4
    //勾选地区
    cy.get('.el-radio__inner').eq(areaIndex).click({
      force: true
    ...
    //选择佛山市
    cy.get('[placeholder="所有市"]').click()
    cy.get('.el-scrollbar__view.el-select-dropdown__list').eq(cityIndex).find('li').eq(guiyangCityindex).click({
      force: true
    })
    cy.server()
    // 拦截接口，使用通配符*拦截更灵活
  ...
  it('009-参与实验室情况-使用标签进行搜索(标签为贵州)', () => {
    ...
    cy.get('.el-select-group').eq(boxIndex).find('li').eq(guizhouTag).click()
    cy.server()
    cy.route('**/service/mgr/evaReport/joinLab?startTime*').as('getLabdata')
    // 拦截请求必须写在visit之前
    cy.visit(urlHost + '#/manage/report-effect/report-effect-partake')
  ...
  it('010-参与实验室情况-使用标签进行搜索(标签为佛山)', () => {
    ...
    cy.get('.el-select-group__wrap').eq(systemTag).find('li>ul>li').eq(foshanRealTag).click({
      force: true
    })
