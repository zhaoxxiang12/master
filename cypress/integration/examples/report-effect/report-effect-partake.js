context('参与实验室情况', () => {
  ...
  it('004-参与实验室情况-获取实验室数据', () => {
    let chooseBox = 1
    cy.get('.el-radio__inner').eq(chooseBox).click({
      force: true
    })
    //点击管理机构
  ...
  it('006-参与实验室情况-使用地区进行搜索(广东省佛山市)', () => {
    let areaIndex = 0
    let boxIndex = 4
    let cityIndex = 4
  ...
  it('007-参与实验室情况-使用地区进行搜索(上海)', () => {
    let boxIndex = 4
    let cityIndex = 4
    let ShanghaiIndex = 1
    let resultIndex = 4
    let closeArea = 0
    cy.get('.el-tag__close.el-icon-close').eq(closeArea).click({
      force: true
    })
    //选择上海
    cy.get('[placeholder="请选择省"]').click({
    ...
    cy.get('.el-scrollbar__view.el-select-dropdown__list').eq(cityIndex).find('li').click({
      force: true
    })
    // 拦截接口，使用通配符*拦截更灵活
    cy.intercept('**/service/mgr/evaReport/joinLab?startTime*').as('getLabdata')
  ...
  it('008-参与实验室情况-使用标签进行搜索(标签为佛山)', () => {
    ...
    cy.get('.el-tag__close.el-icon-close').eq(closeTag).click({
      force: true
    })
    //选择标签为佛山
