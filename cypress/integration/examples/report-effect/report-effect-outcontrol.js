context('失控处理情况', () => {
    ...
    it('012-失控处理情况-按原因查看-获取页面上处理方式数据', () => {
        ...
        let optionsIndex = 8
        let chooseIndex = 0
        let methodOneIndex = 4
        let methodTwoIndex = 5
        let methodThreeIndex = 6
        let methodFourIndex = 7
        let methodFiveIndex = 8
        let methodSixIndex = 9
        let methodSevenIndex = 10
        //点击按原因查看
        cy.get('.effect__search').find('[role="radio"]').eq(2).click()
        //点击原因选择框
        cy.get('[placeholder="请选择"]').eq(boxIndex).click()
        ...
        cy.get('button').contains('搜索').click()
        //获取处理方式1的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodOneIndex).invoke('text')
            .then((data) => {
                let webData = data
        ...
        //获取处理方式2的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodTwoIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
        ...
        //获取处理方式3的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodThreeIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
        ...
        //获取处理方式4的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodFourIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
        ...
        //获取处理方式5的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodFiveIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
        ...
        //获取处理方式6的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodSixIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
        ...
        //获取处理方式7的数据
        cy.get('.table-line__fixed-header+.table-line').find('tbody>tr').eq(0).find('td').eq(methodSevenIndex).invoke('text')
            .then((data) => {
                let webData = data
                methodData.push(webData)
                // cy.log(methodData)
