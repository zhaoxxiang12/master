context('信息互通设置-推送设置', () => {
    ...
    it('004-新建失控告警规则-未上报质控数据', () => {
        ...
        cy.intercept('**/service/mgr/message/rules?*').as('getData')
        //点击保存
        cy.get('.el-button.el-button--primary.el-button--medium').eq(ButtonIndex).click({
            force: true
        })
    ...
    it('010失控告警规则-启用/停用', () => {
        ...
        cy.get('.el-table__fixed-body-wrapper').find('table>tbody>.el-table__row').eq(TurnOnOff).find('.el-button.el-button--text.el-button--medium').eq(index).invoke('text').then((data) => {
            ...
            if (text == '启用') {
                ...
                cy.wait('@getData').then((xhr) => {
                    let status = xhr.response.statusCode
                    //断言状态码
    ...
    it('011-失控告警规则-编辑失控规则', () => {
        ...
        cy.get('.el-table__body').find('.el-table__row').then((data) => {
            let getLength = data.length
            cy.get('.el-table__body').find('.el-table__row').eq(getLength - 1).find('button').eq(editBtn).click({
                force: true
            })
            cy.wait(500)
            // 选择未上报质控数据
            ...
            cy.get('.el-select-dropdown__list li').contains('01:00').click({
                force: true
            })
            //点击保存
            cy.get('.el-button.el-button--primary.el-button--medium').eq(ButtonIndex).click({
