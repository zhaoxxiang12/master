...
context('实验室信息采集表配置', () => {
  ...
  it('001-实验室信息采集表配置-添加表单', () => {
    ...
    //---------字段名称为空不能保存 -------------
    cy.get('.el-input__inner').eq(formBox).clear().type(formName)
    cy.get('.el-button.el-button--primary.el-button--medium').eq(save).click()
  ...
  it('002--实验室信息采集表配置-推送表单', () => {
    ...
    cy.get('.el-table__body').find('.el-table__row').last().find('.el-button.el-button--text.el-button--medium').contains('推送').click()
  ...
  it('003-实验室信息采集表配置-编辑表单', () => {
    ...
    let formBox = 1
    const newFormName = `测试修改名称${Date.now()}`
    //同一表单下列表名称不允许重名
    cy.get('.el-table__body').find('.el-table__row').last().find('.cell').first().invoke('text').then((getText) => {
      ...
      cy.get('.el-button.el-button--primary.el-button--medium').eq(save).click()
      cy.wait(1000)
      cy.get('body').should('contain', '字段名称' + '"' + typeWord + '"' + '重复')
      //修改表单名称
