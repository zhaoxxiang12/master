context('系统日志', () => {
  ...
  it('001-系统日志-级别选择信息进行查询', () => {
    ...
    cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').contains('信息').click()
  ...
  it('002-系统日志-级别选择告警进行查询', () => {
    let eventLevel = 0
    cy.get('.el-input__inner').eq(eventLevel).click()
    cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').contains('警告').click()
    cy.intercept('**/cqb-base-mgr/service/system/log/page?startTime=*').as('getData')
    cy.get('button').contains('搜索').click()
    cy.wait('@getData').then((xhr)=>{
      let responseStatus = xhr.response.statusCode
  ...
  it('003-系统日志-级别选择异常进行查询', () => {
    ...
    cy.wait(500)
    cy.get('.el-input__inner').eq(eventLevel).click()
    cy.get('.el-scrollbar__view.el-select-dropdown__list').find('li').contains('异常').click()
    cy.intercept('**/cqb-base-mgr/service/system/log/page?startTime=*').as('getData')
    cy.get('button').contains('搜索').click()
    cy.wait('@getData').then((xhr)=>{
      let responseStatus = xhr.response.statusCode
