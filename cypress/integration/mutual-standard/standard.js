context('互认标准设置', () => {
  ...
  context('批量设置规则', () => {
    it('cqb-004-批量设置规则', () => {
      ...
      //下拉选择月份2
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq('2').click({
        force: true
      })
      //点击‘CV%判定规则’下拉框
      ...
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq(1).click({
        force: true
      })
      //点击选择部中复选框
      cy.get('.el-checkbox__label').eq(3).click({
        force: true
      })
      //点击部中心下拉框
      ...
      cy.wait(1000)
      //点击复选框
      cy.get('.el-table__body').last().find('.el-table__row').first().find('.el-checkbox').click()
      //点击‘保存’按钮
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        let getLength = getData.length
        cy.log(getLength)
    ...
    it('cqb-005-互认标准设置-添加标准-批量设置规则-不限项目', () => {
      ...
      cy.wait(5000)
      //分类选择新冠病毒核酸检测
      cy.get('.el-button.spec-filter__item.el-button--small').contains('新冠病毒核酸检测').click({
        force: true
      })
      //点击复选框（选择几个项目设置规则）
      cy.wait(1000)
      ...
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('.el-select-dropdown__item').eq(1).click({
        force: true
      })
      //设置消息读取率的合格月数  //选择消息读取率次数-2次(选择多少次修改最后一个eq里面的参数)
  ...
  context('规则保存', () => {
    it('cqb-006-互认标准设置-阈值标准未填写不能保存', () => {
      ...
      //分类选择新冠病毒核酸检测
      cy.get('.el-button.spec-filter__item.el-button--small').contains('常规化学').click({
        force: true
      })
      cy.wait(3000)
      ...
      //点击选择部中复选框
      cy.get('[for="partEqaPassTimes"]').find('[type="checkbox"]').check('', {
        force: true
      })
      //点击部中心下拉框
      cy.get('.el-form.rule-form.el-form--inline').last().findAllByPlaceholderText('不判定').first().click({
        force:true
      })
      //部级选择次数，2次
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq(1).click({
    ...
    it('cqb-007-互认标准设置-连续合格月数未填写不能保存', () => {
      ...
      cy.wait(3000)
      cy.get('.el-input__inner').eq(1).clear().type('测试标准', {
        force: true
      })
      cy.wait(1000)
      //分类选择新冠病毒核酸检测
      ...
      cy.wait(1000)
      //设置阈值标准
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').type('12')
      //点击‘CV%判定规则’下拉框
      cy.get('input[placeholder="合格判定规则"]').eq(0).click({
        force: true
      })
      ...
      //点击部中心下拉框
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('不判定').first().click()
      //部级选择次数，2次
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq(1).click({
        force: true
      })
    ...
    it('cqb-009-互认标准设置-阈值填写不规范(负数大于100，0，特殊符号)不能保存', () => {
      ...
      //设置阈值标准
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').clear().type('-99')
      //设置连续合格月数
      cy.get('input[placeholder="连续合格月数"]').eq(0).click({
        force: true
      })
      ...
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').clear().type('@')
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        let getLength = getData.length
        cy.get('.el-button.el-button--primary.el-button--medium').eq(getLength + 7).click({
          force: true
      ...
      cy.get('body').should('contain', '请填大于0，小于100的值')
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').clear().type('0')
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        let getLength = getData.length
        cy.get('.el-button.el-button--primary.el-button--medium').eq(getLength + 7).click({
          force: true
      ...
      cy.get('body').should('contain', '请填大于0，小于100的值')
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').clear().type(100)
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        let getLength = getData.length
        cy.get('.el-button.el-button--primary.el-button--medium').eq(getLength + 7).click({
          force: true
  ...
  context('模板功能', () => {
    it('cqb-010-互认标准设置-模板名称为空不能保存', () => {
      ...
      // 设置阈值标准
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('0 < 阈值 < 100').type('99')
      //设置连续合格月数
    ...
    it('cqb-011-互认标准设置-保存模板', () => {
      ...
      //设置阈值标准
      cy.get('.el-form.rule-form.el-form--inline').last().findAllByPlaceholderText('0 < 阈值 < 100').type('99')
      //设置连续合格月数
      cy.get('input[placeholder="连续合格月数"]').eq(clickButton).click({
        force: true
      })
      ...
      //点击选择省中复选框
      cy.get('[for="provinceEqaPassTimes"]').find('[type="checkbox"]').check('', {
        force: true
      })
      //点击省中心下拉框
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('不判定').eq(1).click({
        force: true
      })
      //省级选择次数，2次
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq(1).click({
      ...
      cy.get('[for="cityEqaPassTimes"]').find('[type="checkbox"]').check('', {
        force: true
      })
      //点击市中心下拉框
      cy.get('.el-form.rule-form.el-form--inline').eq(2).findAllByPlaceholderText('不判定').last().click({
        force: true
      })
      //市级选择次数，2次
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').eq(1).click({
      ...
      cy.get('.el-input__inner').last().type('自动化测试模板', {
        force: true
      })
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        let getLength = getData.length
    ...
    it('cqb-013-互认标准设置-修改模板(模板名称)', () => {
      ...
      cy.get('.el-table__body').eq(miniList).find('.el-table__row').then((getData) => {
        ...
        cy.get('[aria-label="修改互认规则设置模板"]').find('.el-input__inner').first().clear().type(typeName,{
          force:true
        })
        cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
          let getLength = getData.length
    ...
    it('cqb-014-互认标准设置-使用模板', () => {
      ...
      cy.get('.el-table__body').eq(miniList).find('.el-table__row').then((getData) => {
        ...
        cy.wait(1000)
        //断言(看复选框是否已选中)
        cy.get('[for="provinceEqaPassTimes"]').last().find('.el-checkbox').should('have.class', 'is-checked')
        cy.get('.el-button.el-button--default.el-button--medium').eq(cancel1).click({
          force: true
        })
        cy.get('.el-button.el-button--default.el-button--medium').eq(cancel2).click({
  ...
  context('额外操作', () => {
    ...
    it('cqb-017-互认标准设置-已关联标准的实验室不能再次关联(复选框置灰)', () => {
      ...
      cy.wait(2000)
      //断言复选框是否置灰
      cy.get('.el-checkbox__input').last().should('have.class', 'is-disabled')
      //输入实验室名称
      ...
      cy.wait(2000)
      //断言复选框是否未置灰
      cy.get('.el-checkbox__input').last().should('not.have.class', 'is-disabled')
      cy.get('.el-button.el-button--default.el-button--medium').eq(cancel1).click({
        force: true
      })
      cy.get('.el-button.el-button--default.el-button--medium').eq(cancel2).click({
    ...
    it('cqb-018-互认标准设置-复制互认标准', () => {
      ...
      cy.get('.sd-cfg').find('.el-card.sd-cfg__item.is-hover-shadow').then((getData) => {
        ...
        for (let i = 0; i < getLength; i++) {
          ...
          index = element + i
          //点击保存
          cy.get('.el-button.el-button--primary.el-button--medium').eq(index).click({
            force: true
          })
          //点击弹窗确认
          cy.get('.el-button.el-button--default.el-button--small.el-button--primary ').click({
        ...
        cy.intercept({
          url: '**/cqb-base-mgr/service/mgr/std/yearrecog/*',
          method: 'DELETE'
        }).as('deleteData')
        cy.get('.el-button.el-button--default.el-button--small.el-button--primary.el-button--danger').click({
          force: true
  ...
  context('切换质控主管单位',() => {
    it('cqb-020-互认标准设置-切换质控主管单位(青浦医联体)', () => {
      ...
      cy.get('.el-scrollbar__view.el-select-dropdown__list').last().find('li').contains('青浦医联体').click({
        force: true
      })
