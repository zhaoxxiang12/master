...
context('差异性分析报告', () => {
  ...
  let result
  const queryData = () => {
    let index = Math.random() * 100000
    // return interceptAll('service/mgr/lab/report/monthdiff/new-page?*', 'queryData' )
    cy.intercept(/\/cqb-base-mgr\/service\/mgr\/lab\/report\/monthdiff\/new-page\?[^-]*$/).as('queryData')
    return 'queryData'
  }
  ...
  const pushMany = (rowIndex) => {
    ...
    waitIntercept(batchPushIntercept, pushReport, data => {
      waitIntercept(queryData, data => {
  ...
  context('查询1-8月', () => {
    ...
    it('006-已下载状态栏显示已下载', () => {
      ...
      cy.get('.el-table__body .el-table__row').eq(rowIndex).findByText('已下载').should('exist')
    })
  })
  context('批量推送/批量取消推送', () => {
    before(() => {
