...
context('互认报告和证书管理', () => {
  ...
  const editCertModel = (alias)=>{
    return util.interceptAll('service/mgr/template/saveOrUpdate',alias)
  }
  const waitSave = (cb) => {
    util.waitIntercept(editCertModel('saveCert'),()=>{
      cy.get('.el-dialog__footer').eq(3).find('button').contains('保存').click()
    },cb)
  }
  const visitCertPage = (alias) => { //访问年度互认证书页面
    let labCode = 'gdtest5'
    cy.visitPage('cert-year')
    cy.wait(2000)
    cy.get('.el-form.el-form--inline').findByPlaceholderText('请输入实验室名称或编码').type(labCode)
    util.interceptAll('/service/mgr/mutualRecogReport?*', 'getData')
    cy.wait(5000)
  ...
  const generateCertReport = (month,editModel) => { //生成互认报告以及证书
    ...
    util.interceptAll('/service/mgr/mutualRecogReport/check?*', 'certReport')
    ...
    cy.wait('@addInstrGroup' + number).then((xhr) => {
      ...
      cy.wait('@customerReport' + number).then((xhr) => {
        ...
        cy.get('.el-form').eq(4).find('.compare-group .el-checkbox-group.compare-group__tags .el-icon-close').then((getData) => {
          ...
          util.interceptAll('/service/iqc/compareGroup/*', 'getGroup' + number)
          util.confirmDelete()
  ...
  it('020-互认报告-模板编辑', () => {
    ...
    //标题
    util.elformInput('title', title, () => {

    })
    //副标题
    util.elformInput('subtitle', subTitleName, () => {

    })
    //页眉
    util.elformInput('headerTemplate',headerTemplate,'function',()=>{
      
    })
    //模板名称重复不能保存
    util.elformInput('name', repeatModelName, () => {
    ...
    //副标题为空不能保存
    util.elformInput('subtitle',null,()=>{

    })
    util.elformInput('name',reportMobelName,'function',()=>{
